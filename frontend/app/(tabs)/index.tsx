          ...
          appointments.slice(0, 5).map((apt: any) => (
            <View key={apt.id} style={styles.appointmentItem}>
              <View style={{flex: 1}}>
                ...
                <Text style={[styles.appointmentStatus, { color: apt.status === 'pending' ? '#FF9500' : apt.status === 'confirmed' ? '#34C759' : '#666' }]}>
                  {apt.status === 'pending' ? 'Pendente' : apt.status === 'confirmed' ? 'Confirmado' : apt.status === 'completed' ? 'Concluído' : apt.status === 'cancelled' ? 'Cancelado' : apt.status}
                </Text>
              </View>
