...
export default function Loyalty() {
  ...
  const toggleStatus = async () => {
    try {
      ...
      await api.put('/loyalty/config', { ...config, is_active: newStatus });
      setConfig({ ...config, is_active: newStatus });
      Alert.alert('Sucesso', `Programa ${newStatus ? 'ativado' : 'desativado'}`);
    } catch {
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(); }} tintColor={theme.primary} />}>
        <View style={[styles.configCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
          <View style={styles.configRow}>
            <Text style={[styles.configLabel, { color: theme.textSecondary }]}>R$1 =</Text>
            <Text style={[styles.configValue, { color: theme.text }]}>{config.points_per_real} ponto(s)</Text>
          </View>
          <View style={styles.configRow}>
            <Text style={[styles.configLabel, { color: theme.textSecondary }]}>Meta:</Text>
            <Text style={[styles.configValue, { color: theme.text }]}>{config.redemption_threshold} pts</Text>
          </View>
          <View style={styles.configRow}>
            <Text style={[styles.configLabel, { color: theme.textSecondary }]}>Premio:</Text>
            <Text style={[styles.configValue, { color: theme.text }]}>{config.reward_description}</Text>
          </View>
          
          <View style={styles.configActions}>
            <TouchableOpacity 
              style={[styles.configActionBtn, { backgroundColor: theme.primary }]} 
              onPress={() => setShowConfig(true)}
            >
              <Ionicons name="create-outline" size={16} color="#FFF" />
              <Text style={styles.configActionText}>Editar</Text>
            </TouchableOpacity>
            
            <TouchableOpacity 
              style={[styles.configActionBtn, { backgroundColor: config.is_active ? '#FF3B30' : '#34C759' }]} 
              onPress={toggleStatus}
            >
              <Ionicons name={config.is_active ? "close-outline" : "checkmark-outline"} size={16} color="#FFF" />
              <Text style={styles.configActionText}>
        ...
        {clients.length === 0 ? (
          ...
          clients.map((c: any) => (
            <View key={c.id} style={[styles.clientCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
              ...
              <View style={[styles.progressBar, { backgroundColor: theme.divider }]}>
                <View style={[styles.progressFill, { width: `${Math.min((c.points / config.redemption_threshold) * 100, 100)}%`, backgroundColor: config.is_active ? '#34C759' : theme.textMuted }]} />
              </View>
              <Text style={[styles.progressText, { color: theme.textMuted }]}>{Math.max(config.redemption_threshold - c.points, 0)} pontos para resgate</Text>
              <View style={styles.clientActions}>
                <TouchableOpacity style={[styles.actionBtn, { borderColor: theme.primary }]} onPress={() => loadHistory(c.client_phone)}>
